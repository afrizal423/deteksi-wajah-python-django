...
def detect(request):
    ...
    while(True):
        ...
        for(x,y,w,h) in faces:
            ...
            getId,conf = rec.predict(gray[y:y+h, x:x+w]) # ini akan memprediksi id dari wajah 
            ...
            # disini akan menampilkan nomor dibawah wajah 
...
def create_dataset(request):
    ...
    # Mendeteksi wajah
    # membuat cascade image classifier
    faceDetect = cv2.CascadeClassifier(BASE_DIR+'/ml/haarcascade_frontalface_default.xml')
    # mengambil gambar dari webcam dan akan diproses dan mendeteksi wajah 
    # akan mengcapture video id , untuk webcam  untuk webcam sebagian besar waktu 0.
    cam = cv2.VideoCapture(0)
